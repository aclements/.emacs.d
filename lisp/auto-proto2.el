...
(defun auto-proto-do-function-prototype (text decl)
  (let ((decl-name (or (auto-proto-decl-name decl)
        ...
        (search-path (auto-proto-default-search-path decl))
        (done nil))
    (while (not done)
      ;; Filter the search path to existing files (and files that are
      ;; open in buffers but not necessarily saved yet)
      (setq search-path (remove-if-not
                         (lambda (path)
                           (or (file-exists-p path)
                               (find-buffer-visiting path)))
                         search-path))
      ;; Search for existing declarations in the search path
      (let (existing bufs-to-kill)
        (save-excursion
          (dolist (path search-path)
            (let ((buf (find-buffer-visiting path)))
              (unless buf
                (setq buf (find-file-noselect path t t))
                (push buf bufs-to-kill))
              (with-current-buffer buf
                (setq existing
                      (auto-proto-find-decl decl-name #'auto-proto-is-fnproto))
                (return)))))
        (when existing
          ;; Prompt can update done, search-path, and static-flag
          ;; XXX
      )))))
