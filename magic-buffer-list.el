...
(defun magic-buffer-list-pop-up (buffer)
  (let ((in-buffer (eq (current-buffer) buffer))
    ...
    ;; magic-buffer-list-un-pop-up changes the current window, so if
    ;; we're trying to pop up in a different window, undo the change.
    (unless (and in-buffer prev-window)
